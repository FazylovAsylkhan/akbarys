...
module.exports.sendMail = (event, context, callback) => {
  ...
  const structureLetter = `
    ...
    <html>
      ...
      ${
        bodyData.link
          ? '<h1>Страница "Карьера"</h1>'
          : '<h1>Страница "Контакты"</h1>'
      }
        <b>Name: </b><em>${bodyData.name}</em><br/> 
        <b>Email: </b><em>${bodyData.email}</em><br/> 
        <b>Message: </b><em>${bodyData.message}</em><br/> 
      </body>
    </html>
  `;
